...
from graspnetAPI.my_graspnet_eval_add_language_frozen import GraspGroup, GraspNetEval
...
from models.GRASPNET.real_my_graspnet_add_language_frozen_with_rgb_clip_rn50 import GraspNet, pred_decode, real_pred_decode
from dataset.real_my_graspnet_dataset_add_language_frozen_with_rgb_no_resize import GraspNetDataset, minkowski_collate_fn
...
parser.add_argument('--checkpoint_path', help='Model checkpoint path', default='/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb_my_clip_rn50_frozen_no_resize_batch_size_4/minkuresunet_add_language_frozen_epoch_with_rgb15.tar', required=False)
parser.add_argument('--dump_dir', help='Dump dir to save outputs', default='/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb_my_clip_rn50_frozen_no_resize_batch_size_4/dump_epoch15_add_language_frozen_with_rgb', required=False)
...
# epoch_type = 'minkuresunet_add_language_frozen_epoch_with_rgb10.tar'
# cfgs.checkpoint_path = '/media/ama/data0/gz/graspnet/graspness_unofficial/' + model_type + '/' + epoch_type
# cfgs.dump_dir = '/media/ama/data0/gz/graspnet/graspness_unofficial/' + model_type + '/' + f'dump_epoch{epoch_type[-6:-4]}_add_language_frozen_with_rgb'
...
def inference(split, epoch):
    cfgs.dump_dir = '/media/ama/data0/gz/graspnet/graspness_unofficial/logs/' + model_type + '/' + 'real_scene_dump_epoch' + str(epoch).zfill(2) + '_add_language_frozen_with_rgb'
    cfgs.checkpoint_path = '/media/ama/data0/gz/graspnet/graspness_unofficial/logs/' + model_type + '/' + 'minkuresunet_add_language_frozen_epoch_with_rgb' + str(epoch).zfill(2) + '.tar'
    if not os.path.exists(cfgs.dump_dir):
        os.mkdir(cfgs.dump_dir)
    test_dataset = GraspNetDataset(cfgs.dataset_root, split=split, num_points=cfgs.num_point,
    ...
    for batch_idx, batch_data in enumerate(tqdm(test_dataloader, desc='inference')):
        ...
        for i in range(cfgs.batch_size):
            ...
            if i >= end_points['batch_good_list'].shape[0]:
                break
            if end_points['batch_good_list'][i] ==1:
                ...
                # save grasps
                save_dir = os.path.join(cfgs.dump_dir, 'SORTED', scene_list[data_idx], ann_list[data_idx])
                save_path = os.path.join(save_dir, obj_list[data_idx] + '.npy')
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                gg.save_npy(save_path)

                save_dir_res = os.path.join(cfgs.dump_dir, 'CLS', split, scene_list[data_idx], ann_list[data_idx])
...
def evaluate(split, epoch, TOP_K=5,re=''):
    dump_dir = '/media/ama/data0/gz/graspnet/graspness_unofficial/logs/' + model_type + '/' + re +  'dump_epoch' + str(
        epoch).zfill(2) + '_add_language_frozen_with_rgb/' + 'SORTED'
    ...
    return_cls_acc= True
    use_origin = False
    ge = GraspNetEval(root=cfgs.dataset_root,split=split,TOP_K=TOP_K)
    # split = 'test_seen'
    t1 = time.time()
    if split =='test_seen':
    ...
    #                                                                 return_cls_acc=return_cls_acc,
    #                                                                 use_origin=use_origin, proc=6)
    cfgs.dump_dir = '/media/ama/data0/gz/graspnet/graspness_unofficial/logs/' + model_type + '/' + re + 'dump_epoch' + str(
        epoch).zfill(2) + '_add_language_frozen_with_rgb'
    save_dir_res = os.path.join(cfgs.dump_dir, 'TOP_K_{}'.format(TOP_K), 'res_{}.txt'.format(split))
    with open(save_dir_res, 'w') as f:
        for list_ in res_list:
            f.write(str(list_))
            f.write('\n')

    save_dir_ap = os.path.join(cfgs.dump_dir, 'TOP_K_{}'.format(TOP_K), 'ap_{}.txt'.format(split))
    np.savetxt(save_dir_ap, np.array([ap]))
    # print('test_seen_time:',time.time()-t1)

    save_dir_res = os.path.join(cfgs.dump_dir, 'TOP_K_{}'.format(TOP_K), 'cls_acc_{}.txt'.format(split))
    with open(save_dir_res, 'w') as f:
        for list_ in cls_acc_list:
            f.write(str(list_))
            f.write('\n')

    save_dir_ap = os.path.join(cfgs.dump_dir, 'TOP_K_{}'.format(TOP_K), 'cls_ap_{}.txt'.format(split))
...
if __name__ == '__main__':
    inference('test_similar', 10)
    # inference('test_train', 10)
    # inference('test_similar', 15)
    # inference('test_train', 15)
    # inference('test_similar', 8)
    # inference('test_train', 8)
    # inference('test_similar', 12)
    # inference('test_train', 12)
    # inference('test_similar', 6)
    # inference('test_train', 6)
    # inference('test_similar', 20)
    # inference('test_train', 20)
    # evaluate('test_seen', 10, TOP_K=1)
    # evaluate('test_seen', 10, re='real_',TOP_K=1)
