...
from models.GRASPNET.graspnet_add_language_frozen_with_rgb_resnet50_test import GraspNet, real_pred_decode
...
parser.add_argument('--checkpoint_path', help='Model checkpoint path', default='/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb_resnet50_frozen_no_resize/minkuresunet_add_language_frozen_epoch_with_rgb10.tar', required=False)
parser.add_argument('--dump_dir', help='Dump dir to save outputs', default='/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb_resnet50_frozen_no_resize/real_dump_epoch10_add_language_frozen_with_rgb', required=False)
...
def inference(split):
    ...
    # Init the model
    net = GraspNet(seed_feat_dim=cfgs.seed_feat_dim, is_training=False, img_encoder_frozen=True)
    # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    net.to(device)
    # Load checkpoint
    ...
    for batch_idx, batch_data in enumerate(tqdm(test_dataloader, desc='inference')):
        ...
        with torch.no_grad():
            ...
            t1 = time.time()
            grasp_preds, res = real_pred_decode(end_points)
...
def evaluate(dump_dir, split, TOP_K=5):
    return_grasp_acc = True
    return_cls_acc= True
    use_origin = False
    ge = GraspNetEval(root=cfgs.dataset_root, camera=cfgs.camera, split='test', TOP_K=TOP_K)
    # split = 'test_seen'
    t1 = time.time()
    if split =='test_seen':
    ...
    #                                                                 return_cls_acc=return_cls_acc,
    #                                                                 use_origin=use_origin, proc=6)
    data_root = os.path.join(cfgs.dump_dir, 'TOP_K_{}'.format(TOP_K), split)
    if not os.path.exists(data_root):
        os.makedirs(data_root)
    save_dir_res = os.path.join(data_root, 'res_{}_{}.txt'.format(cfgs.camera, split))
    ...
    save_dir_ap = os.path.join(data_root,'ap_{}_{}.txt'.format(cfgs.camera, split))
    np.savetxt(save_dir_ap, np.array([ap]))
    # print('test_seen_time:',time.time()-t1)

    save_dir_res = os.path.join(data_root, 'cls_acc_{}_{}.txt'.format(cfgs.camera, split))
    with open(save_dir_res, 'w') as f:
        for list_ in cls_acc_list:
            f.write(str(list_))
            f.write('\n')
    f.close()
    save_dir_ap = os.path.join(data_root, 'cls_ap_{}_{}.txt'.format(cfgs.camera, split))
...
if __name__ == '__main__':
    ...
    # inference('test_novel')
    # inference('test_evaluate')
    # evaluate(os.path.join(cfgs.dump_dir, 'SORTED'), 'test_evaluate', 5)
    # evaluate(os.path.join(cfgs.dump_dir, 'SORTED'), 'test_evaluate', 3)
    # inference('test_train')
    # evaluate(os.path.join(cfgs.dump_dir,'SORTED'), 'test_train')
    # inference('test_similar')
    # inference('test_seen')
    ...
    # # inference('test_novel')
    # time1 = time.time()
    # evaluate(os.path.join(cfgs.dump_dir, 'SORTED'), 'test_seen', 5)
    # print(time.time() - time1)
    #
    # time1 = time.time()
    # evaluate(os.path.join(cfgs.dump_dir, 'SORTED'), 'test_similar', 5)
    # print(time.time() - time1)
    #
    # time1 = time.time()
    # evaluate(os.path.join(cfgs.dump_dir, 'SORTED'), 'test_novel', 5)
    # print(time.time() - time1)

    time1 = time.time()
    evaluate(os.path.join(cfgs.dump_dir, 'SORTED'), 'test_seen', 3)
    print(time.time() - time1)

    time1 = time.time()
    evaluate(os.path.join(cfgs.dump_dir, 'SORTED'), 'test_similar', 3)
    print(time.time() - time1)

    time1 = time.time()
    evaluate(os.path.join(cfgs.dump_dir, 'SORTED'), 'test_novel', 3)
    print(time.time() - time1)
    # if cfgs.infer:
    # inference('test_similar')
    # inference('test_novel')
