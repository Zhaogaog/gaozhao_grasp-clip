...
from models.graspnet_add_language_frozen_test import GraspNet, pred_decode
from dataset.graspnet_dataset_add_language_frozen import GraspNetDataset, minkowski_collate_fn


parser = argparse.ArgumentParser()
parser.add_argument('--dataset_root', default='/media/ama/data0/gz/graspnet/graspnet', required=False)
parser.add_argument('--checkpoint_path', help='Model checkpoint path', default='/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn/CHECKPOINT/minkuresunet_add_language_frozen_epoch15.tar', required=False)
parser.add_argument('--dump_dir', help='Dump dir to save outputs', default='logs/log_kn/dump_epoch15_add_language_frozen', required=False)
parser.add_argument('--seed_feat_dim', default=512, type=int, help='Point wise feature dim')
parser.add_argument('--camera', default='kinect', help='Camera split [realsense/kinect]')
parser.add_argument('--num_point', type=int, default=15000, help='Point Number [default: 15000]')
parser.add_argument('--batch_size', type=int, default=56, help='Batch Size during inference [default: 1]')
...
def inference():
    ...
    net.to(device)
    # Load checkpoint
    checkpoint1 = torch.load('/media/ama/data0/gz/graspnet/graspness_unofficial/np15000_graspness1e-1_bs4_lr1e-3_viewres_dataaug_fps_14D_epoch10.tar')
    ...
    for batch_idx, batch_data in enumerate(test_dataloader):
        ...
        for i in range(cfgs.batch_size):
            ...
            # save grasps
            save_dir = os.path.join(cfgs.dump_dir, scene_list[data_idx], cfgs.camera, ann_list[data_idx])
...
def evaluate(dump_dir):
    ...
    #         f.write('\n')
    # f.close()
    # save_dir_ap = os.path.join(cfgs.dump_dir, 'cls_ap_{}_{}.npy'.format(cfgs.camera, split))
    # np.save(save_dir_ap, cls_ap)
    # # print('test_seen_time:', time.time() - t1)
    ...
    for scene in cls_res_list:
        id_list = []
        path = os.path.join(cfgs.dataset_root, 'scenes', 'scene_' + str(scene[0][0]['scene']).zfill(4),
                            'object_id_list.txt')
