...
parser.add_argument('--dump_dir', help='Dump dir to save outputs', default='/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb_resnet50_frozen_no_resize/dump_epoch10_add_language_frozen_with_rgb', required=False)
...
def evaluate(dump_dir, split, TOP_K=5):
    ...
    return_cls_acc= True
    use_origin = False
    ge = GraspNetEval(root=cfgs.dataset_root, camera=cfgs.camera, split='test', TOP_K=TOP_K, vis=False)
    # split = 'test_seen'
    t1 = time.time()
    if split =='test_seen':
