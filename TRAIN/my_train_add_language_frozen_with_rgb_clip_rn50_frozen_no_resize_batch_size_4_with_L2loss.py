...
parser.add_argument('--log_dir', default='/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb_my_clip_rn50_frozen_no_resize_batch_size_4_large_dataset_with_L2loss')
...
cfgs.checkpoint_path = '/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb_my_clip_rn50_frozen_no_resize_batch_size_4_large_dataset_with_L2loss_1e-05/minkuresunet_add_language_frozen_epoch_with_rgb16.tar'
cfgs.resume = True
CHECKPOINT_PATH = cfgs.checkpoint_path if cfgs.checkpoint_path is not None and cfgs.resume else None
alpha = 0.00001
cfgs.log_dir = '/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb_my_clip_rn50_frozen_no_resize_batch_size_4_large_dataset_with_L2loss_'+str(alpha)
...
def train_one_epoch():
    ...
    for batch_idx, batch_data_label in enumerate(tqdm(TRAIN_DATALOADER, desc='Train')):
        ...
        loss, end_points = get_loss(end_points, net, alpha=alpha)
        loss.backward()
        optimizer.step()
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, cfgs.max_epoch):
        if epoch ==0:
            save_dict = {'epoch': epoch , 'optimizer_state_dict': optimizer.state_dict(),
                         'model_state_dict': net.state_dict()}

            torch.save(save_dict, os.path.join(cfgs.log_dir,
                                               cfgs.model_name + '_add_language_frozen_epoch_with_rgb' + str(
                                                   epoch).zfill(2) + '.tar'))
        EPOCH_CNT = epoch
