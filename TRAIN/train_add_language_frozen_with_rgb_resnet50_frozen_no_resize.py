...
from models.GRASPNET.graspnet_add_language_frozen_with_rgb_resnet50 import GraspNet
from models.LOSS.loss_add_language_frozen import get_loss
from dataset.graspnet_dataset_add_language_frozen_with_rgb_no_resize import GraspNetDataset, minkowski_collate_fn, load_grasp_labels
...
parser.add_argument('--log_dir', default='/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb_resnet50_frozen_no_resize')
...
# cfgs.checkpoint_path = '/media/ama/data0/gz/graspnet/graspness_unofficial/logs/log_kn_with_rgb/minkuresunet_add_language_frozen_epoch_with_rgb02.tar'
cfgs.resume = True
...
net = GraspNet(seed_feat_dim=cfgs.seed_feat_dim, is_training=True, img_encoder_frozen=True)
...
for (name, param) in net.clip.clip_rn50.named_parameters():
    param.requires_grad = False
# state_dict = net.clip.state_dict()
for (name, param) in net.clip.named_parameters():
    if 'resnet' in name:
        param.requires_grad = False
...
def train_one_epoch():
    ...
    net.clip.resnet_conv1.eval()
    net.clip.resnet_bn1.eval()
    net.clip.resnet_layer1.eval()
    net.clip.resnet_layer2.eval()
    net.clip.resnet_layer3.eval()
    net.clip.resnet_layer4.eval()
