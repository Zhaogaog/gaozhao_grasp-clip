...
def minkowski_collate_fn(list_data):
    ...
    print(coordinates_batch.shape)
    #将在内存中不连续存储转的数组化为连续存储的数组，处理更快。
    coordinates_batch = np.ascontiguousarray(coordinates_batch, dtype=np.int32)
    print(type(coordinates_batch))
    # coordinates_batch[0:10, :] = coordinates_batch[0:10, :]
    # features_batch[0:10, :] = features_batch[0:10,:]
    # print(coordinates_batch[0:10,:])
    # print(features_batch[0:10,:])

    coordinates_batch, features_batch, _, quantize2original = ME.utils.sparse_quantize(
        coordinates_batch[0:10,:], features_batch[0:10,:], return_index=True, return_inverse=True)

    print(quantize2original)
    print(coordinates_batch)
