...
class GraspNetDataset(Dataset):
    def __init__(self, root, grasp_labels=None, split='train', num_points=20000,
        ...
        if split == 'train':
            self.sceneIds = list(range(0,108)) + list(range(135, 142))
        ...
        for x in tqdm(self.sceneIds, desc='Loading data path and collision labels...'):
            ...
            for img_num in range(255):
                self.camK_path.append(os.path.join(root, 'scenes', x, 'camK.npy'))
                self.camera_poses_path.append(os.path.join(root, 'scenes', x, 'camera_poses.npy'))
                self.imgpath.append(os.path.join(root, 'scenes', x, 'rgb', str(img_num).zfill(4) + '.png'))
    ...
    def get_data(self, index, return_raw_cloud=False):
        ...
        if self.split == 'test':
            ...
            index = (self.obj_lists[index]['scene_id'] - 27) * 255 + self.obj_lists[index]['ann_id']
