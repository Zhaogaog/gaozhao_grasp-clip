...
from data_utils import CameraInfo, transform_point_cloud, create_point_cloud_from_depth_image, get_workspace_mask, my_create_point_cloud_from_depth_image
from models._clip import  load_clip_no_resize


class GraspNetDataset(Dataset):
    def __init__(self, root, grasp_labels=None, split='train', num_points=20000,
                 voxel_size=0.005, remove_outlier=False, augment=False, load_label=True):
        ...
        if split == 'train':
            self.sceneIds = list(range(0,142))
            # self.sceneIds = list(range(5,6))
        elif split == 'test':
            self.sceneIds = list(range(100, 190))
        elif split == 'test_seen':
            self.sceneIds = list(range(100, 130))
        elif split == 'test_similar':
            self.sceneIds = list(range(1, 5))
        ...
        if 'test' in self.split :
            self.obj_lists_path = os.path.join(root, 'obj_lists', 'obj_lists'+ '_' + self.split + '.json')
            self.obj_lists = []
            self.scene_lists = []
            self.ann_lists = []
            self.obj_ids = []
        ...
        for x in tqdm(self.sceneIds, desc='Loading data path and collision labels...'):
            # self.camK_path.append(os.path.join(root, 'scenes', x, 'camK.npy'))
            # self.camera_pose_path.append(os.path.join(root, 'scenes', x, 'camera_poses.npy'))
            for img_num in range(1):
                self.camK_path.append(os.path.join(root, 'scenes', x, 'camK.npy'))
                # self.camera_poses_path.append(os.path.join(root, 'scenes', x, 'camera_poses.npy'))
                self.imgpath.append(os.path.join(root, 'scenes', x, 'rgb', str(img_num).zfill(4) + '.jpg'))
                self.depthpath.append(os.path.join(root, 'scenes', x, 'depth', str(img_num).zfill(4) + '.npy'))
                self.labelpath.append(os.path.join(root, 'scenes', x, 'label', str(img_num).zfill(4) + '.png'))
                self.obj_idxs_path.append(os.path.join(root, 'scenes', x, 'label', str(img_num).zfill(4) + '.json'))
                # self.metapath.append(os.path.join(root, 'scenes', x,  'meta', str(img_num).zfill(4) + '.mat'))
                # self.graspnesspath.append(os.path.join(root, 'graspness', x, str(img_num).zfill(4) + '.npy'))
                #去掉字符串两边的空格
    ...
    def get_data(self, index, return_raw_cloud=False):
        ...
        if self.split == 'test':
            index = (self.obj_lists[index]['scene_id']- 100 )*255+self.obj_lists[index]['ann_id']
        elif self.split == 'test_seen':
            index = (self.obj_lists[index]['scene_id'] - 100) * 255 + self.obj_lists[index]['ann_id']
        elif self.split == 'test_similar':
            index = (self.obj_lists[index]['scene_id'] - 1) * 1 + self.obj_lists[index]['ann_id']
        elif self.split == 'test_novel':
            index = (self.obj_lists[index]['scene_id'] - 160) * 255 + self.obj_lists[index]['ann_id']
        elif self.split == 'test_train':
            index = (self.obj_lists[index]['scene_id'] - 0) * 255 + self.obj_lists[index]['ann_id']
        elif self.split == 'test_evaluate':
            index = (self.obj_lists[index]['scene_id'] - 100) * 255 + self.obj_lists[index]['ann_id']
        # scene_id = torch.div(index, 256, rounding_mode='trunc')
        # ann_id = index%256
        depth = np.load(self.depthpath[index])
        seg = np.array(cv2.imread(self.labelpath[index], -1))
        #mata的作用 提供class类别 内参数 比例因子 恢复点云
        # meta = scio.loadmat(self.metapath[index])
        image_numpy = self.preprocess(Image.open(self.imgpath[index]).convert('RGB')).numpy()
        scene = self.scenename[index]
        try:
            #meta和label对应，从1-88
            # obj_idxs = meta['cls_indexes'].flatten().astype(np.int32)
            with open(self.obj_idxs_path[index],'r') as f:
                obj_idxs_dict = eval(f.read())
            obj_idxs = list(obj_idxs_dict.values())
            # poses = meta['poses']
            # intrinsic = meta['intrinsic_matrix']
            intrinsic = np.load(self.camK_path[index])
            factor_depth = 0.001
        ...
        #     camera_poses = np.load(self.camera_poses_path[index])
        #     # align_mat = np.load(os.path.join(self.root, 'scenes', scene, 'cam0_wrt_table.npy'))
        #     trans = camera_poses[self.frameid[index]]
    ...
    def get_data_label(self, index):
        ...
        # image =
        # img = np.array(Image.open(self.imgpath[index]).convert('RGB'))
        image_numpy = self.preprocess(Image.open(self.imgpath[index]).convert('RGB')).numpy()
        ...
        ret_dict = {'point_clouds': cloud_sampled.astype(np.float32),
                    ...
                    # 'pack_obj_id': np.array([49]),
                    'mask_remove_outlier': mask.astype(np.int64),
                    'mask_sampled': mask_sampled.astype(np.int64),
                    'img': image_numpy,
                    }
        return ret_dict
