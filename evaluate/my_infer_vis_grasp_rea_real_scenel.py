...
from utils.data_utils import CameraInfo, my_create_point_cloud_from_depth_image, my_get_workspace_mask, create_point_cloud_from_depth_image, get_workspace_mask, transform_point_cloud

parser = argparse.ArgumentParser()
parser.add_argument('--dataset_root', default='/media/ama/data0/gz/graspnet/graspnet_real')
...
def data_process(scene,index, obj_idxs=None):
    ...
    image_numpy = preprocess(Image.open(os.path.join(root, 'scenes', scene_id, 'rgb', index + '.jpg')).convert('RGB')).numpy()
    ...
    # cloud = transform_point_cloud(cloud.reshape([720*1280,3]),trans).reshape([720,1280,3])
    mask = depth_mask
...
def inference(data_input,save_real=True):
...
if __name__ == '__main__':
    ...
    cfgs.dump_dir = '/media/ama/data0/gz/graspnet/graspness_unofficial/logs/' + model_type + '/' + 'real_scene_dump_epoch' + str(
        epoch).zfill(2) + '_add_language_frozen_with_rgb'
    real_dump_dir = '/media/ama/data0/gz/graspnet/graspness_unofficial/logs/' + model_type + '/' + 'real_real_scene_dump_epoch' + str(
        epoch).zfill(2) + '_add_language_frozen_with_rgb'
    ...
    if split == 'test_train':
        scene_list = get_scene_name(category)
    print(scene_list)
    for scene in tqdm(range(1,5), desc='scene inference'):
    # for scene in tqdm(list(scene_list.keys()), desc='scene inference'):
        ...
        for index in tqdm(range(0, 1), desc='ann inference'):
            # if index not in [58,59,63,84,89,93,106,108,125,154,185]:#teddy bear
            # if index not in [28,42,69,90,106,112,116,126,131,152,176,190,196,197,198,227]:#baseball bat
            #     continue
            # if index not in [1,4,5,6,7,8,10]:#baseball bat
            #     continue
            obj_idxs = None
            # obj_idxs = [10]
            while cfgs.infer:
            ...
            if cfgs.vis:
                ...
                for obj in os.listdir(os.path.join(cfgs.dump_dir, 'SORTED', scene_id, str(index).zfill(4))):
                    ...
                    gg = GraspGroup().from_npy(os.path.join(cfgs.dump_dir, 'SORTED', scene_id, str(index).zfill(4), obj))
                    gg = gg.nms()
                    gg = gg.sort_by_score()
                    if gg.__len__() > 30:
                        gg = gg[:30]
                    ...
                    for i in range(grasp_array.shape[0]):
                        ...
                        camera_pose = np.load(os.path.join(root, 'scenes', scene_id, 'camera_poses.npy'))[index]
