...
from models.GRASPNET.my_graspnet_add_language_frozen_with_rgb_clip_rn50 import GraspNet, pred_decode
...
if __name__ == '__main__':
    ...
    scene_list = {'scene_0300':'0001'}
    if split == 'test_train':
        scene_list = get_scene_name(category)
    # scene_list = {'scene_0036': '0074'}
    # scene_list = {'scene_0123': '0003'}
    print(scene_list)
    for scene in tqdm(list(scene_list.keys()), desc='scene inference'):
        ...
        for index in tqdm(index_list, desc='ann inference'):
            ...
            if cfgs.vis:
                ...
                # for obj in os.listdir(os.path.join(cfgs.dump_dir, scene_id, cfgs.index)):
                #     if show_one:
                #         if cfgs.obj not in obj:
                #             continue
                #     gg = np.load(os.path.join(cfgs.dump_dir, scene_id, cfgs.index, obj))
                #     gg = GraspGroup(gg)
                #     gg = gg.nms()
                #     gg = gg.sort_by_score()
                #     if gg.__len__() > 30:
