...
class my_clip(nn.Module):
    ...
    def _load_clip(self):
        model, preprocess = load_clip("RN50")
    ...
    def _build_decoder(self):
        # language
        self.lang_fuser1 = fusion.names[self.lang_fusion_type](input_dim=torch.div(self.input_dim, 1, rounding_mode='trunc'))
        self.lang_fuser2 = fusion.names[self.lang_fusion_type](input_dim=torch.div(self.input_dim, 2, rounding_mode='trunc'))
        self.lang_fuser3 = fusion.names[self.lang_fusion_type](input_dim=torch.div(self.input_dim, 4, rounding_mode='trunc'))
        ...
        #     nn.ReLU(True)
        # )
        self.up1 = Up(512+256, torch.div(512, self.up_factor, rounding_mode='trunc'), self.bilinear)
        # self.lat_fusion1 = FusionConvLat(input_dim=1024 + 512, output_dim=512)

        self.up2 = Up(256+128, torch.div(256, self.up_factor, rounding_mode='trunc'), self.bilinear)
        # self.lat_fusion2 = FusionConvLat(input_dim=512 + 256, output_dim=256)

        self.up3 = Up(128+64, torch.div(128, self.up_factor, rounding_mode='trunc'), self.bilinear)
        ...
        self.layer3 = nn.Sequential(
            ConvBlock(16, [8, 8, 8], kernel_size=3, stride=1, batchnorm=self.batchnorm),
            IdentityBlock(8, [8, 8, 8], kernel_size=3, stride=1, batchnorm=self.batchnorm),
            nn.UpsamplingBilinear2d(scale_factor=2),
        )
