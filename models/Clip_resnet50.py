...
class my_clip(nn.Module):
    ...
    def _load_resnet_50(self):
        # model = models.resnet50(weights=models.ResNet50_Weights.DEFAULT)
        model = models.resnet50(pretrained=False)
        model.load_state_dict(torch.load('/media/ama/data0/gz/graspnet/graspness_unofficial/WEIGHT/resnet50-11ad3fa6.pth'))
        model.fc = nn.Sequential()
        # model.
        self.resnet_conv1 = model.conv1
        self.resnet_bn1 = model.bn1
    ...
    def _encode_image(self, x):
        if self.use_50:
            ...
            with torch.no_grad():
                t1 = time.time()
                img_encoding, img_im = self.clip_rn50.visual.prepool_im(x)
