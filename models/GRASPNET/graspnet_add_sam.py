...
def pred_decode(end_points):
    ...
    for i in range(batch_size):
        if end_points['batch_good_list'][i] == 1:
            ...
            prec = objectness_graspable_label.mean().item()
            num = torch.sum(objectness_graspable_label).item()
            if num != M_POINT:
                print(num)
            ...
            grasp_score = grasp_score.view(-1, 1)#[M_POINT, 1]
            ...
            grasp_width = 1.2 * end_points['grasp_width_pred'][i] / 10.#乘1.2的作用
            ...
            # merge preds
            grasp_height = 0.02 * torch.ones_like(grasp_score)#height是什么
            # obj_ids = -1 * torch.ones_like(grasp_score)
            obj_ids= end_points['pack_obj_id'][i].item() * torch.ones_like(grasp_score)
            res.append({'num_right': num, 'prec': prec})
            grasp_preds.append(
