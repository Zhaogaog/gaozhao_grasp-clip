...
from models.modules_no_more_text_no_more_mlp import ApproachNet, GraspableNet, CloudCrop, SWADNet, Lang_selectNet_with_rgb
...
class GraspNet(nn.Module):
    def __init__(self, cylinder_radius=0.05, seed_feat_dim=512, is_training=True, img_encoder_frozen=True, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"), split='train'):
        super().__init__()
        self.device = device
        ...
        self.clip = my_clip(img_encoder_frozen=self.img_encoder_frozen, device=self.device, split=split)

        self.backbone = MinkUNet14D(in_channels=3, out_channels=self.seed_feature_dim, D=3)
        #抓取点网络
        self.graspable = GraspableNet(seed_feature_dim=self.seed_feature_dim)
        self.lang_select_with_rgb = Lang_selectNet_with_rgb(
            seed_feature_dim=self.seed_feature_dim + self.seed_feature_dim)
    ...
    def forward(self, end_points):
        ...
        # print('img_feat——_time', time.time() - t1)
        # del img_feat
        # text_feat, _, _ = self.clip_text_encoder.encode_sentence(pack_obj_id_batch)
        #点云编码器
        #（4，15000，3）点的坐标
        t1 = time.time()
        ...
        for i in range(B):
            ...
            cur_text_feat = text_feat[i]
            #1024
            if cur_seed_xyz.shape[1] != 0:
                # print('训练合格的点数为：', cur_seed_xyz.shape[1])
                seed_text_feat_graspable.append(cur_text_feat)
        ...
        seed_text_feat_graspable = torch.stack(seed_text_feat_graspable, 0)
        # print('1024_time',time.time()-t1)
        # seed_features_graspable_cat = torch.cat([seed_features_graspable,seed_img_features_graspable],1)
        #当进一步把点数减少后，如果剩余的点数不足1024怎么办。减少为128
        end_points['objectness_graspable_label'] = seed_objectness_graspable
        end_points['xyz_graspable'] = seed_xyz_graspable
        end_points['graspable_count_stage1'] = graspable_num_batch / B #平均每个可以被抓起来的点数
        t1 = time.time()
        # if seed_features_graspable.shape[0]==0:
        #     print(end_points['pack_obj_id'])
        #     print(
        #         '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        end_points = self.lang_select_with_rgb(seed_features_graspable, seed_img_features_graspable, seed_text_feat_graspable, end_points)
        # print('lang_select_time:', time.time() - t1)
        if not self.is_training:
            ...
            group_features = self.crop(seed_xyz_graspable.contiguous(), seed_features_graspable.contiguous(),
                                       grasp_top_views_rot)
...
def pred_decode(end_points):
    ...
    for i in range(batch_size):
        if end_points['batch_good_list'][i] == 1:
            ...
            # obj_ids = obj_id
            grasp_preds[i] = torch.cat([grasp_score, grasp_width, grasp_height, grasp_depth, grasp_rot, grasp_center, obj_ids], axis=-1)[lang_select_objectness_mask]
            res[i] = {'num_label':num_label, 'num_pred': num_pred, 'acc':acc, 'prec':prec, 'recall':recall}
        else:
            grasp_preds[i] = {}
            res[i] = {}
...
def real_pred_decode(end_points):
    ...
    for i in range(batch_size):
        if end_points['batch_good_list'][i] == 1:
            ...
            res[i] = {}
            real_grasp_preds = {}
    return grasp_preds, res, real_grasp_preds