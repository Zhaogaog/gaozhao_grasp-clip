...
split = 'test_seen'
class my_evaluate():
    def __init__(self, root_dir, model_type, epoch_chosen, obj_top_k, split,scene_id):
        if split == 'train':
            self.sceneIds = list(range(100))
        elif split == 'test':
            self.sceneIds = list(range(100, 190))
        elif split == 'test_seen':
            self.sceneIds = list(range(scene_id, scene_id
                                      +1 ))
        elif split == 'test_similar':
            self.sceneIds = list(range(scene_id, scene_id+1))
    ...
    def evaluate_strict(self, object_id) :
        # ann_grasp_acc = []
        obj_ratio_list = []
        abs_obj_list = []
        abs_real_obj_list = []
        ...
        for x in self.sceneIds :
            ...
            for ann in sorted(os.listdir(real_scene_path)) :
                    ...
                    with open(pre_ann_path, 'r') as f:
                        for line in f:
                            ...
                            if json.loads(line)['label'][0] != 'NONE':
                                ...
                                # grasp_acc_dict[json.loads(line)['pre'][0][-4:]] = json.loads(line)['acc']
                # ann_ratio_list = []
                temp_list = []
                for key in real_grasp_acc_dict.keys() :
                    ...
                    if key in grasp_acc_dict.keys():
                        # if '0020' in key:
                        if object_id in key:
                            apple_max[ann] = np.mean(grasp_acc_dict[key])
                        sum.append(grasp_acc_dict[key])
            ...
            # print(max)
            apple_max = sorted(apple_max.items(), key=lambda x: x[1], reverse=True)
            print(apple_max)
...
obj_top_k = 'TOP_K_1'
scene_id = 404
evaluation = my_evaluate(root_dir, model_type, 'dump_epoch10_add_language_frozen_with_rgb', obj_top_k, split,scene_id=scene_id)
...
with open('/media/ama/data0/gz/graspnet/graspnet_sim/obj_lists/all_category_' + split + '_dict.json', 'r') as f:
    CAT_ID = json.loads(f.read())
print(input_classes)
for object_name in thing_classes:

    object_id = str(CAT_ID[object_name]).zfill(4)
    print(object_name)
    print('{} {} {}, abs_real_obj,abs_obj,obj_ratio  '.format(model_type, epoch_chosen, obj_top_k), evaluation.evaluate_strict(object_id=object_id))
