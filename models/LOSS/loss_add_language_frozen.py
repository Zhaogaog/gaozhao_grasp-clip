...
def get_loss(end_points):
    ...
    # if torch.sum(end_points['batch_good_list']) == 0:
    #     print('不合格')
    #     return objectness_loss, end_points
    ...
    loss = objectness_graspable_loss
...
def compute_objectness_graspable_loss(end_points):
    ...
    focal_loss = Focal_Loss(weight = torch.tensor([0.75,0.25], device=objectness_graspable_score.device))
    ...
    end_points['stage1_objectness_acc'] = (objectness_pred == objectness_graspable_label.long()).float().mean()
    end_points['stage1_objectness_prec'] = (objectness_pred == objectness_graspable_label.long())[
        objectness_pred == 1].float().mean()
    end_points['stage1_objectness_recall'] = (objectness_pred == objectness_graspable_label.long())[
