...
def my_create_point_cloud_from_depth_image(depth, intrinsic, organized=True):
    K_INV = np.linalg.inv(intrinsic)
    points_2d = np.argwhere(depth > float('-inf'))[:, [1, 0]]
    depth_flatten = depth.flatten()*0.001
    ...
    depth_expand = np.expand_dims(depth_flatten, axis=0).repeat(3, axis=0).T
    cloud = points_3d * depth_expand

    if organized:
        cloud = cloud.reshape([720, 1280, 3])
    return cloud
...
def my_transform_point_cloud(cloud, transform, format='4x4'):
    ones = np.ones((cloud.shape[0], 1), dtype=cloud.dtype)
    points_3d_nml = np.hstack((cloud, ones))
    points_3d_world = (np.dot(np.linalg.inv(transform), points_3d_nml.T).T)[:, 0:3]
...
def get_workspace_mask(cloud, seg, trans=None, organized=True, outlier=0):
    ...
    # print(np.max(cloud[:,2]), np.min(cloud[:,2]))
...
def my_get_workspace_mask(cloud, seg, trans=None, organized=True, outlier=0):
    """ Keep points in workspace as input.
    ...
    if organized:
        ...
        seg = seg.reshape(h * w)
    if trans is not None:
        cloud = my_transform_point_cloud(cloud, trans)
