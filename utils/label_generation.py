...
def process_grasp_labels(end_points):
    ...
    for i in range(batch_size):
        ...
        for obj_idx, pose in enumerate(poses):
            grasp_points = end_points['grasp_points_list'][i][obj_idx]  # (Np, 3) #每个物体上的理论抓取点
            grasp_scores = end_points['grasp_scores_list'][i][obj_idx]  # (Np, V, A, D)#每个点每个方向每个角度每个距离的抓取分数
            grasp_widths = end_points['grasp_widths_list'][i][obj_idx]  # (Np, V, A, D)#每个点每个方向每个角度每个距离的抓取宽度
            ...
            grasp_points_trans = transform_point_cloud(grasp_points, pose, '3x4')#每个物体上的理论抓取点在世界系
            grasp_views_trans = transform_point_cloud(grasp_views, pose[:3, :3], '3x3')#抓取方向与世界坐标系中的概念。
            ...
            view_inds = knn(grasp_views_trans_, grasp_views_, k=1).squeeze() - 1 #世界系下的300个方向对应原来物体系的300个方向
            ...
            grasp_views_rot_merged.append(grasp_views_rot_trans)#旋转矩阵的作用
    ...
    batch_grasp_points = torch.stack(batch_grasp_points, 0)  # (B, Ns, 3)#1024个点对应的标签值
    ...
    #300个方向对应的标签
    end_points['batch_grasp_point'] = batch_grasp_points #1024个点对应标签中的抓取点
    end_points['batch_grasp_view_rot'] = batch_grasp_views_rot #1024个点300个方向对应的旋转矩阵。
    end_points['batch_grasp_score'] = batch_grasp_scores#1024个点对应标签中的抓取分数
    end_points['batch_grasp_width'] = batch_grasp_widths#1024个点对应标签中的抓取宽度
    end_points['batch_grasp_view_graspness'] = batch_grasp_view_graspness #1024个点300个抓取方向的graspness
    return end_points


def match_grasp_view_and_label(end_points):
    """ Slice grasp labels according to predicted views. """#从300个
    ...
    #最佳方向的标签
    end_points['batch_grasp_score'] = top_view_grasp_scores  # (B, Ns, A, D)#top view 标签 score
    end_points['batch_grasp_width'] = top_view_grasp_widths  # (B, Ns, A, D)#top view 标签 width
